 ...
 * 
 * @param {bool} [pugOptions.disabled]
 * Do not add pug. Defaults to false.
 * 
 ...
 */
export default (gruntConfig, targetName, pugOptions = null) => {
  if (pugOptions === null) {
    pugOptions = {};
  }
  if (pugOptions.disabled || false) {
    return [];
  }
  gruntConfig["pug"] = gruntConfig["pug"] || {};
  const newPugTask = {
    options: pugOptions.options,
    files: [{
      expand: true,
      cwd: pugOptions.sourcePath || "webres",
      src: "**/*.pug",
      dest: pugOptions.outputPath || join("dist", targetName),
      ext: pugOptions.fileSuffix || ".html",
    }],
  };
  const taskName = `pug:${targetName}`;
  if (gruntConfig["pug"][targetName] !== undefined) {
    throw new Error(`Task ${taskName} already defined in Grunt configuration`);
  }
  gruntConfig["pug"][targetName] = newPugTask;
  return [`pug:${targetName}`];
};