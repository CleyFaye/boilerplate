...
export type RawOptValue = undefined | boolean | string;
...
const handleOptionString = (rawValue: RawOptValue): string => {
  if (typeof rawValue !== "string") throw new Error("Missing string argument");
  return rawValue;
};

/** Process a number-type argument */
const handleOptionNumber = (rawValue: RawOptValue): number => {
  if (typeof rawValue !== "string") throw new Error("Missing number argument");
  const asNumber = Number.parseFloat(rawValue);
  if (Number.isNaN(asNumber)) throw new Error("Not a number");
  return asNumber;
};

/** Process a boolean-type argument */
const handleOptionBoolean = (rawValue: RawOptValue): boolean => {
  if (typeof rawValue !== "boolean") throw new Error("Missing boolean argument");
  return rawValue;
};

/** Process one option from the list of expected arguments */
const handleOptionDef = (optionDef: OptionDefinition, rawValue: RawOptValue): OptValue => {
  if (rawValue === undefined) {
    if (optionDef.defaultValue === undefined) throw new Error("Missing option");
    return optionDef.defaultValue;
  }
  switch (optionDef.type) {
    case OptType.STRING:
      return handleOptionString(rawValue);
    case OptType.NUMBER:
      return handleOptionNumber(rawValue);
    case OptType.BOOLEAN:
      return handleOptionBoolean(rawValue);
  }
};

/** Get a value from the command line */
const getFromOptionProvider = (keyName: string, optionProvider: OptionsProvider): RawOptValue => {
...
const getTypeInfoString = (optionType: OptType, kebabName: string): string => {
  switch (optionType) {
    case OptType.STRING:
      return "=<string value>";
    case OptType.NUMBER:
    ...
    case OptType.BOOLEAN:
      return ` (use --no-${kebabName} to disable)`;
  }
};

/** Display help for a single parameter */
const displayHelpForConfigKey = (keyName: string, optionDef: OptionDefinition): void => {
...
export const displayHelp = (configDef: ConfigDefinition): void => {
  outputHelp("Accepted command line arguments:");
  Object.keys(configDef).forEach((configKey) =>
    displayHelpForConfigKey(configKey, configDef[configKey]),
  );
};
...
export const getOptionsFromGruntCLI = (
  ...
  const result: Opts = {};
  Object.keys(configDef).forEach((keyName) => {
    try {
